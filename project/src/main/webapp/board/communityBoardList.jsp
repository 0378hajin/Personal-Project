...
<th id = "hover" style = "background :#627062;"><a class = "sellist" href= "<%=request.getContextPath()%>/board/communityBoardList.do">전체</a></th>
<th id = "hover"><a class = "sellist" href= "<%=request.getContextPath()%>/board/communityBoardList_answer.do">질문</a></th>
<th id = "hover"><a class = "sellist" href= "<%=request.getContextPath()%>/board/communityBoardList_info.do">정보</a></th>
<th id = "hover"><a class = "sellist" href= "<%=request.getContextPath()%>/board/communityBoardList_talk.do">잡담</a></th>
</tr>
</table>
<br>
...
<tr class = "tab22">
	...
	<td class = "tab22" style = "text-align : center;">
	<%if(bv.getKind().equals("info")){ %>
	정보
	<% } else if (bv.getKind().equals("answer")){ %>
	질문
	<% } else if (bv.getKind().equals("talk")) {%>
	잡담
	<% } else if (bv.getKind().equals("reply")) {%>
	답변
	<%} %>
	</td>
	...
	%>
	<%if(bv.getKind().equals("info") || bv.getKind().equals("talk") || bv.getKind().equals("reply")){ %>
	<a href = "<%=request.getContextPath()%>/board/communityBoard.do?bidx=<%=bv.getBidx()%>">
	<% } else if (bv.getKind().equals("answer")){ %>
	<a href = "<%=request.getContextPath()%>/board/communityBoard_answer.do?bidx=<%=bv.getBidx()%>">
	<%} %>
...
<br>
<input type = "button" name = "btn" value = "글 쓰기" onclick = "location.href = '<%=request.getContextPath()%>/board/communityBoard_write.do'" style = "font-size : 17px;" id ="btn" class = "wbtn">
<br>
...
<tr>
	...
	<%
	if(pm.isPrev() == true){
		out.println("<a href = '"+request.getContextPath()+"/board/communityBoardList.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>이전</a>");
	}	
	...
	for(int i = pm.getStartPage(); i <= pm.getEndPage(); i++){
		...
		out.println("<a href = '"+request.getContextPath()+"/board/communityBoardList.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>"+i+"</a>");
	}
	...
	if(pm.isNext() && pm.getEndPage() > 0) {
		out.println("<a href = '"+request.getContextPath()+"/board/communityBoardList.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>다음</a>");
	}
...
<td style = "width : 100%;">
<form name = "frm" id = "searchbar" action = "<%=request.getContextPath()%>/board/communityBoardList.do" method = "post">
