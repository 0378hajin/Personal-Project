...
function reply1() {
	...
	fm.action = "<%=request.getContextPath()%>/board/communityBoard_answerReplyAction.do";
	fm.method = "post";
	fm.submit;
}
...
<tr>
<td class = "kindwrite" style = "width : 3%">
...
<input type = "button" name = "answer" value = "답변 하기" class = "lbtn" onclick = "location.href = '<%=request.getContextPath()%>/board/communityBoard_reply.do?bidx=<%=bv.getBidx()%>&originbidx=<%=bv.getOriginbidx()%>&depth=<%=bv.getDepth() %>&level_=<%=bv.getLevel_()%>'">
...
<span id = "hi">환영합니다!</span>
<span id = "nickname"><%=session.getAttribute("membernickname") %> 님!</span>
