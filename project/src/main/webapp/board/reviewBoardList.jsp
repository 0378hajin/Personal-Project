...
<tr class = "tab22">
	...
	<a href = "<%=request.getContextPath() %>/board/reviewBoard.do?bidx=<%=bv.getBidx()%>"><%=bv.getSubject() %></a>
	</td>
...
<br>
<input type = "button" name = "btn" value = "글 쓰기" onclick = "location.href = '<%=request.getContextPath()%>/board/reviewBoard_write.do'" style = "font-size : 17px;" id ="btn" class = "wbtn">
<br>
...
<tr>
	...
	<%
	if(pm.isPrev() == true){
		out.println("<a href = '"+request.getContextPath()+"/board/reviewBoardList.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>이전</a>");
	}	
	...
	for(int i = pm.getStartPage(); i <= pm.getEndPage(); i++){
		...
		out.println("<a href = '"+request.getContextPath()+"/board/reviewBoardList.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>"+i+"</a>");
	}
	...
	if(pm.isNext() && pm.getEndPage() > 0) {
		out.println("<a href = '"+request.getContextPath()+"/board/reviewBoardList.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>다음</a>");
	}
...
<td style = "width : 100%;">
<form name = "frm" id = "searchbar" action = "<%=request.getContextPath()%>/board/reviewBoardList.do" method = "post">
