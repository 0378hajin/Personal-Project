...
<tr style = "text-align : center; background : #627062; color : white; height : 10%;" class = "tab22">
...
<tr class = "tab22" style = "height : 8%;">
	<td class = "tab22"><%=bv.getBidx() %></td>
	<td class = "wit">
	<a href = "<%=request.getContextPath() %>/board/newsBoard.do?bidx=<%=bv.getBidx()%>"><%=bv.getSubject()%></a>
	
	</td>

	<td class = "tab22"><%=bv.getMembernickname() %></td>

	<td class = "tab22"><%=bv.getWriteday() %></td>
</tr>
<% } %>
</table>
<br>
<input type = "button" name = "btn" value = "글 쓰기" onclick = "location.href = '<%=request.getContextPath()%>/board/newsBoard_write.do'" style = "font-size : 17px;" id ="btn" class = "wbtn">
<br>
...
<tr>
	...
	<%
	if(pm.isPrev() == true){
		out.println("<a href = '"+request.getContextPath()+"/board/newsBoardList.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"'>이전</a>");
	}	
	...
	for(int i = pm.getStartPage(); i <= pm.getEndPage(); i++){
		...
		out.println("<a href = '"+request.getContextPath()+"/board/newsBoardList.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"'>"+i+"</a>");
	}
	...
	if(pm.isNext() && pm.getEndPage() > 0) {
		out.println("<a href = '"+request.getContextPath()+"/board/newsBoardList.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"'>다음</a>");
	}
...
<td style = "width : 100%;">
<form name = "frm" id = "searchbar" action = "<%=request.getContextPath()%>/board/newsBoardList.do" method = "post">
