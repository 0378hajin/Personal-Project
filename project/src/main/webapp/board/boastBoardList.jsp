...
<table style = "width : 100%; height: 30%; border-collapse : collapse; font-family : Arial;" border = "1">
	<tr style = "text-align : center; background : #627062; color : white;">
		<td style = "width : 10%">번호</td>
		<td style = "width : 30%">시스템 사진</td>
		<td style = "width : 30%">제목 </td>
		<td style = "width : 30%">작성자 | 작성일</td>
	</tr>
<% for(BoardVo bv : alist) {%>
	<tr style = "text-align : center;">
		...
		<img src = "<%=request.getContextPath()%>/files/<%=bv.getFilename()%>" style = "width : 80%;">
		<% } %>
		</td>
		<td>
		<a href = "<%=request.getContextPath()%>/board/boastBoard.do?bidx=<%=bv.getBidx()%>"><%=bv.getSubject() %></a>
		</td>
		<td><%=bv.getMembernickname() %>&nbsp;   |   &nbsp;<%=bv.getWriteday() %></td>
	</tr>
<% } %>
</table>
<br>

<input type = "button" name = "btn" value = "글 쓰기" onclick = "location.href = '<%=request.getContextPath()%>/board/boastBoard_write.do'" style = "font-size : 17px;" id ="btn" class = "wbtn">
<br>
...
<tr>	
	...
	<%
	if(pm.isPrev() == true){
		out.println("<a href = '"+request.getContextPath()+"/board/boastBoardList.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>이전</a>");
	}	
	...
	for(int i = pm.getStartPage(); i <= pm.getEndPage(); i++){
		...
		out.println("<a href = '"+request.getContextPath()+"/board/boastBoardList.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>"+i+"</a>");
	}
	...
	if(pm.isNext() && pm.getEndPage() > 0) {
		out.println("<a href = '"+request.getContextPath()+"/board/boastBoardList.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>다음</a>");
	}
...
<tr>
<td style = "width : 100%;">
<form name = "frm" id = "searchbar" action = "<%=request.getContextPath()%>/board/boastBoardList.do" method = "post">
